...
// CORS pre Chrome extension - BEZ OBMEDZENÍ
app.use(cors({
    origin: '*', // Povoľte všetky origins
    credentials: false,
...
// Middleware pre overenie extension ID (voliteľné)
const validateExtension = (req, res, next) => {
    const extensionId = req.headers['x-extension-id'];
    
    // Ak nie je extension ID, stále povolte request (pre testovanie)
    if (!extensionId) {
        console.log(`⚠️ No extension ID from ${req.ip} - allowing for testing`);
        return next();
    }
    
    if (!CONFIG.ALLOWED_EXTENSIONS.includes(extensionId)) {
        console.log(`⚠️ Unknown extension: ${extensionId} from ${req.ip} - allowing for testing`);
        return next();
    }
...
app.get('/', (req, res) => {
    res.json({
        ...
        note: 'This is a public proxy server for ClipSmart extension'
    });
});
...
app.listen(PORT, () => {
    ...
    console.log(`⚠️  NOTE: This is a PUBLIC server - no authentication required`);
});
