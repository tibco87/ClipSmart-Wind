...
// CORS pre Chrome extension s Vercel Authentication
app.use(cors({
    origin: function (origin, callback) {
        // Povoľte len Chrome extensions a váš domén
        if (!origin || 
            origin.startsWith('chrome-extension://') || 
            origin.includes('clipsmart') ||
...
// Middleware pre overenie extension ID
const validateExtension = (req, res, next) => {
    const extensionId = req.headers['x-extension-id'];
    
    if (!extensionId) {
        console.warn(`🚫 Missing extension ID from ${req.ip}`);
        return res.status(401).json({ 
            error: 'Missing extension ID',
            message: 'Chýba ID rozšírenia'
        });
    }
    
    if (!CONFIG.ALLOWED_EXTENSIONS.includes(extensionId)) {
        console.warn(`🚫 Unauthorized extension: ${extensionId} from ${req.ip}`);
        return res.status(403).json({ 
            error: 'Unauthorized extension',
            message: 'Nepovolené rozšírenie'
        });
    }
    
    console.log(`✅ Authorized extension: ${extensionId}`);
    next();
};
...
app.get('/health', (req, res) => {
    const health = {
        status: 'OK',
        timestamp: new Date().toISOString(),
        service: 'ClipSmart Translation Proxy (Authenticated)',
        version: '1.0.0',
...
app.get('/', (req, res) => {
    res.json({
        ...
        note: 'This server requires Vercel authentication'
    });
});
...
app.listen(PORT, () => {
    console.log('🚀 ClipSmart Translation Proxy (Authenticated)');
    ...
    console.log(`🔐 NOTE: This server works with Vercel authentication`);
});
